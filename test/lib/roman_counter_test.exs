defmodule TheLeanCafe.RomanCounterTest do
  ...
  test "counts votes from presence metadata" do
    ...
    assert TheLeanCafe.RomanCounter.value(presence_list, 12345) == 1
  end

  test "skips votes that don't match timestamp" do
    presence_list = %{"fred" => %{metas: [%{last_vote: [12345, "+"]}]}}
    assert TheLeanCafe.RomanCounter.value(presence_list, 12346) == 0
  end
  ...
  test "values neutral votes" do
    presence_list = %{"fred" => %{metas: [%{last_vote: [12345, "="]}]}}
    assert TheLeanCafe.RomanCounter.value(presence_list, 12345) == 0
  end
  ...
  test "values mixed votes" do
    ...
    assert TheLeanCafe.RomanCounter.value(presence_list, 12345) == 2
  end
  ...
  test "users to json" do
    ...
    assert TheLeanCafe.RomanCounter.users_to_json(presence_list, 12345) ==
      [
